...
class UM_myCRED_Login_Hooks extends myCRED_Hook {
	...
	public function award_points_login( $user_id ) {
		...
		// Limit
		if ( $this->over_hook_limit( 'um_user_login', 'um-user-login', $user_id ) ) return;
		...
		$prefs = $this->prefs; ?>
			...
			<?php foreach( $this->um_hooks as $hook => $k ):?> 
				...
				<ol>
					...
					<li>
						<div class="h2"><input type="text" name="<?php echo $this->field_name(  array( $hook, 'log' )  ); ?>" id="<?php echo $this->field_id(  array( $hook, 'log' )
						 ); ?>" value="<?php echo $prefs[ $hook ]['log']; ?>" class="long" /></div>
					</li>
					<li>
