...
class Groups_Main_API {
				...
				if ( empty( $search_in ) && empty( $search_keyword )  ){
					...
					if ( 'everyone' == $invite_people ) {
						...
						$main_query = $wpdb->prepare(
							...
							FROM {$wpdb->users} 
							WHERE ID NOT IN( SELECT user_id1 FROM {$table_name} WHERE group_id = %d {$search_in} ) ORDER BY ID DESC {$paginate}",
							$group_id
						);
	...
	function load_more_users() {
		...
		$args = $this->get_members( $group_id, $status, array(), $limit, $offset, $search_keyword, $custom_filter );
