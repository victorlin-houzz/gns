...
function um_groups_search_users( $args ) {
	...
	wp_enqueue_script( 'um_groups' );
	wp_enqueue_style( 'um_groups' );
	$um_groups_invite_fields = get_post_meta( $args['group_id'], '_um_groups_invites_fields', true );
	...
	if( !empty( $um_groups_invite_fields ) ){
		foreach ( $um_groups_invite_fields as $k => $v ){
			UM()->members()->show_filter( $v );
		}
	}
	echo "</form>";
	echo "</div>";
