...
class UM_Online_API {
	...
	function __construct() {
		...
		add_filter( 'um_members_directory_filter_dropdown_options',  array( $this, 'directory_filter_dropdown_options' ), 10, 1 );
	...
	function messaging_show_online_dot() {
		$status = $this->is_online( um_user('ID') ) ? 'online' : 'offline';
		echo '<span class="um-online-status ' . $status . '"><i class="um-faicon-circle"></i></span>';
	}
	...
	/**
	 * @param array $conversation JSON data
	 *
	 * @return array
	 */
	function messaging_online_status( $conversation ) {
		$conversation['online'] = $this->is_online( um_user('ID') );
		return $conversation;
	}
