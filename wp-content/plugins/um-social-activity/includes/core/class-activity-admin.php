    ...
    function manage_um_activity_posts_custom_column($column_name, $id) {
        switch ($column_name) {
            ...
            case 'a_author':
                ...
                echo '<a href="'. um_user_profile_url() . '" target="_blank" title="'. um_user('display_name') .'" class="authorimg">' . get_avatar( um_user('ID'), 32 ) . '</a>';
                break;
            ...
            case 'a_action':
                ...
                if ( UM()->Activity_API()->api()->get_wall( $id ) && UM()->Activity_API()->api()->get_author( $id ) != UM()->Activity_API()->api()->get_wall( $id ) ) {
                    ...
                    echo '<div class="um-admin-activity-resp"><i class="um-icon-forward"></i><a href="'. um_user_profile_url(). '" target="_blank">' . get_avatar(  um_user('ID'), 32 )  . um_user('display_name') . '</a></div>';
                }
                break;
