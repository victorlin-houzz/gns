...
class Messaging_GDPR {
	...
	function render_download_chat_link( $message_to, $user_id ) {
		...
		<a href="<?php echo $this->get_download_url( $response['conversation_id'] ) ?>"
	...
	function download_file() {
		...
		if ( ! UM()->Messaging_API()->api()->can_download( $user_id, $_GET['conversation_id'] ) ) {
			return;
		}
		...
		if ( $_GET['conversation_id'] == 'all' ) {
			...
			$messages = UM()->Messaging_API()->api()->get_conversation_messages( $_GET['conversation_id'] );
