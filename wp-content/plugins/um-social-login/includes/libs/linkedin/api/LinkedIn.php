...
class LinkedIn
{
    ...
    const OAUTH_BASE = 'https://www.linkedin.com/oauth/';

    const SCOPE_BASIC_PROFILE = 'r_basicprofile'; // Name, photo, headline, and current positions
    const SCOPE_LITE_PROFILE = 'r_liteprofile'; // Name, photo, headline, and current positions
    ...
    public function getLoginUrl(array $scope = array(), $state = null)
    {
        ...
        $url = self::OAUTH_BASE . "v2/authorization?response_type=code&client_id={$this->_config['api_key']}&scope={$scope}&state={$state}&redirect_uri=" . urlencode($this->_config['callback_url']);
    ...
    public function getAccessToken($authorization_code = null)
    {
        ...
        $data = $this->_makeRequest( self::OAUTH_BASE .  'v2/accessToken?' . $tmp_params, array(), self::HTTP_METHOD_GET, array('x-li-format: json'));
