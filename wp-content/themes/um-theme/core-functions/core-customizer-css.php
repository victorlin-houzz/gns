...
function um_theme_customizer_head_styles() {
	...
	$css .= '
		...
		.meta,.comments-link a,.post-meta__time,.single.post-meta,.comment-reply-link,.post-meta__author,.comment-edit-link{color:' . sanitize_hex_color( $website_meta ) . ';}
	...
	if ( class_exists( 'UM' ) ) :
		...
		$css .='
			...
			.um-account-side li{background-color:' . sanitize_hex_color( $defaults['um_theme_um_account_tab_color'] ) . ' !important;}
			.um-account-side li a span.um-account-title{color:' . sanitize_hex_color( $defaults['um_theme_um_account_tab_text_color'] ) . '!important;}

			.um-account-side li a span.um-account-icon,
			.um-account-side li a.current span.um-account-icon{
				color: ' . sanitize_hex_color( $defaults['um_theme_um_account_tab_icon_color'] ) . '!important;
			}

			.um-account-side li a:hover {
				background-color:' . sanitize_hex_color( $defaults['um_theme_um_account_tab_hover_color'] ) . '!important;
			}

			.um-account-side li a:hover .um-account-title,
			.um-account-side li a:hover .um-account-icon,
			.um-account-side li a.current:hover span.um-account-icon {
				color:' . sanitize_hex_color( $defaults['um_theme_um_account_tab_text_hover_color'] ) . '!important;
			}
	...
	if ( function_exists( 'um_messaging_plugins_loaded' ) ) :
		...
		$css .= '
			...
			.um-message-conv .um-message-conv-item.active{
				border-right:5px solid ' . sanitize_hex_color( $um_ext_pm_button_bg_color ) . ' !important;
			}
	...
	if ( function_exists( 'um_groups_plugins_loaded' ) ) :
		...
		$css .= '
			...
			.um-group-item,
			.group-grid-inner {
				background-color:' . sanitize_hex_color( $defaults['um_theme_ext_groups_bg_color'] ) . ';
			}
			...
			.group-grid-inner{
				border-color:' . sanitize_hex_color( $defaults['um_theme_ext_group_tab_border_color'] ) . ';
			}
